...
<div class="container">
  ...
  <div class="card">
    <div class="card-header">
      <h2>Transaksi</h2>
      <span style="background:#ede9fe;color:#6366f1;padding:2px 10px;border-radius:20px;font-size:12px;font-weight:700;">{{ $transactions->total() }} total</span>
    </div>
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr>
          <td><strong style="color:#6366f1;font-size:13px;">{{ $trx->invoice_number }}</strong></td>
          <td>{{ $trx->customer_name ?? 'Umum' }}</td>
          <td><span class="badge" style="background:#ede9fe;color:#6366f1;">{{ $trx->items->count() }} item</span></td>
          <td><strong>Rp {{ number_format($trx->total,0,',','.') }}</strong></td>
          <td style="text-transform:capitalize;font-size:13px;">{{ $trx->payment_method }}</td>
          <td>
            ...
            @else <span class="badge badge-warning">{{ $trx->status }}</span>
            @endif
          </td>
